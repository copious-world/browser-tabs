...
class MLLightInterface extends TopicDataManager {
    ...
    set_shared_shared_structures(change_consumer,table) {
        this._processor.change_consumer = change_consumer
    ...
    prune_words(urls_to_words) {
        ...
        for ( let url in urls_to_words ) {
            ...
            if ( w_list.length === 0 ) {
                ...
                w_list.sort((w1,w2) => {
                    //
                    let w_keeper_1 = this.word_list_get(word)
                    let w_keeper_2 = this.word_list_get(word)
    ...
    #_inject_topics(tabs) {
        ...
        tabs.forEach(tab => {
            ...
            for ( let word of words ) {
                let word_keeper = this.word_list_add_word(topic)
        ...
        for ( let word in this._word_list ) {
            let word_keeper = this.word_to_all_topics(word)
            this.add_to_topics(word,word_keeper._count)
        }
        //
