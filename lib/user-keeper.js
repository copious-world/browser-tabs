...
function pair_list_to_map(pair_list,key_ky,value_ky) {
    let pair_map = {}
    for ( let pair of pair_list ) {
        pair_map[pair[key_ky]] = pair[value_ky]
    }
}
...
class UserKeeper_tabs extends TopicDataManager {
    ...
    change_ml_interface(new_ml_i) {
        ...
        try {
            ...
            this.load_table(this)  // set back to self and call the ml_interface
            return true
        } catch (e) {
    ...
    injest(tabs) {
        ...
        try {
            tabs.forEach(tab => {
                ...
                if ( !url ) return
                if ( url === "about:blank" ) return  // handle special cases for exclusion
                let url_domain = unpack_domain(url)        // organize url links by hostname...
                this.all_domains_add_url(url_domain,url,title)
            })
            // USE ml_interface
    ...
    injest_window(tabs) {
        ...
        try {
            ...
            this._windows.push({                    // ADD THE DOMAIN TO THE LIST OF WINDOWS...
                ...
                "descr" : lead_title,
                "tabs" :  tabs //  pair_list_to_map(tabs,"url","title")   
            })
            // //
            let tmap = this._all_domains[lead_domain]      // get the array we just made by reference
            //
            tabs.forEach(tab => {           // push real links without duplication on to the list of linkss associated with the topic.
                let {url, title} = tab      // tab data structure originate in extensions (tab-senses.js)
                let aurl = url
                if ( !aurl ) return
                if ( aurl === "about:blank" ) return  // handle special cases for exclusion
                //
                if ( tmap[aurl] === undefined ) {
                    tmap[aurl] = title
                }
            })
            //
