...
class TopicDataManager {
    ...
    all_topics_add_url(topic,url,title) {
        let url_map = this._all_domains[topic]
        if ( url_map ) {
            if ( url_map[url] === undefined ) {
                url_map[url] = title
                let topic_obj = this.domains_add(topic)
                if ( topic_obj ) {
    ...
    all_topics_remove_url(topic,url) {
        ...
        if ( url_map ) {
            if ( url_map[url] ) {
                delete url_map[url]
                let topic_obj = this.domains_subtract(topic)
                if ( topic_obj ) {
    ...
    topics_revision(topics_all) {
        for ( let topic in topics_all ) {
            ...
            this._all_domains[topic] = new_urls  // a map domains to titles
        }
    }
    ...
    domains_add(domain,init_weight,title) {
        ...
        if ( !has_domain ) {
            let topic_obj = topic_maker(domain,init_weight)
            topic_obj.descr = title
